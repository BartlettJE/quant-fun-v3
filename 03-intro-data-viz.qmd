...
In the data set, there is a variable for The Authentic Happiness Inventory (AHI): `ahiTotal`. Plot the new variable and try to recreate the customisation layers before checking the solution below. It make take some trial-and-error to get some features right, so do not worry if it does not immediately look the same. 
...
summarydata <- mutate(summarydata, # mutate to change columns in the data
         sex = as.factor(sex), # save sex as a factor, using the original values of sex
         educ = as.factor(educ),
...
```{r, eval = FALSE}
 ggsave("participant_sex_barplot", 
...
- overriding x labels but not order 
