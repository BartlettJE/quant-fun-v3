...
There is currently much debate (and hype) surrounding smartphones and their effects on well-being, especially with regard to children and teenagers. We will be looking at data from ['A Large-Scale Test of the Goldilocks Hypothesis'](https://journals.sagepub.com/doi/abs/10.1177/0956797614542274){target="_blank"} [@przybylski_large-scale_2017] who studied English adolescents. 
...
```{r meanwb, echo = FALSE, message=FALSE}
wemwbs <- wellbeing %>%
...
```{r combined, fig.cap = "Relationship between wellbeing and screentime usage by technology and weekday", message = FALSE}
...
```{r plots, echo = FALSE, fig.cap="Relationship between mean wellbeing and smartphone use by gender", message = FALSE}
...
```{r fig.cap= "Adjusted homogeneity plot that will produce reference line"}
...
* Using the code from Power and Effect Size calculate the minimum effect size we could reliably observe given our sample size and design but for 99% power. Report this to 2 decimal places `r fitb(".00", "0.00")`
