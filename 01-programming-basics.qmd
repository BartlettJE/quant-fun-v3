...
The first thing to do is to get ourselves orientated in RStudio. You will know in RStudio is that it has a number of windows. RStudio has a **`r glossary("console", display = "console window")`** that you can try out code in (appearing as the bottom left window in Figure @fig-img-rstudio, there may be a **`r glossary("script")`** editor (top left - not show when you first open RStudio), a window showing functions and objects you have created in the **`r glossary("environment", display = "Environment")`** tab (top right window in the figure), and a window that shows plots, files packages, and help documentation (bottom right).
