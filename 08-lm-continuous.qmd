...
1. Load the following packages (three of these are new, so revisit [Chapter 1](#install-tidy) if you need a refresher of installing R packages, but remember not to do this on the university computers / online server):
...
`r glossary("Centered predictors", display = "Centering", def = "")` predictors is where you change the values of your predictor, but not their scale. Typically, this means substracting the mean of your predictor from each observation. This changes how you interpret the intercept of your regression model. 
...
Assumptions 5-7 require diagnostic checks on the `r glossary("residuals", def = "")` from the model. The residuals are the difference between your observed values in the data and the values your model expects given it's assumptions. If you remember back, we highlighted the model output mentioned `Residuals:` and they are saved within the model object.
...
In the code below, we use a format you will be less familiar with as all these functions come from base R. If you just run the code for the diagnostic plots `plot(lm_redistribution)`, each one gets individually printed to your Plots window. Here, we create a 2x2 panel to show them all together, then turn it back off. 
...
To isolate each plot, we can use the `which` argument. Plot 1 is a residuals vs fitted plot and helps us check linearity by showing the residuals on the y-axis and the fitted values on the x-axis. 
...
Here, you are looking out for a roughly flat horizontal red line. Common patterns to look out if there is a problem are when the lines has an obvious curve to look like a hump or several bends to look like an S. 
...
When there are problems with normality, you are looking for obvious deviations, like a the points curving around in a banana shape, or snaking around like an S.
...
When there is heteroscedasticity, the characteristic patterns to look out for are a kind of arrow shape where there is a wide spread of points at one end and decreases to a denser range, or a bow tie where there is a wide spread of points at each end and dense in the middle. 

### Checking all the assumptions

There is a fourth plot which you could examine using `plot(lm_redistribution, which = 4)`, but we are going to leave that until Chapter 11. It shows influential values or outliers which have the biggest individual effect on the model residuals. It is not technically an assumption of linear regression, but has knock on effects for the other assumptions. 
...
Our model explained a statistically significant amount of variance in our outcome (adjusted $R^2$ = .49, F(1, 303) = 291.70, *p* < .001). Fairness and satisfaction was a negative predictor, where for every 1-unit increase we expect support for redistribution to decrease by 0.40 ($b_1 = -0.40$, 95% CI = [-0.44, -0.35], *p* < .001)."
