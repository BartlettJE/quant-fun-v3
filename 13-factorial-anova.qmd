...
- We first isolate the standardised residuals of the model object. We must convert it to a data frame and rename the two columsn. 
...
You can use `tidy()` to tidy up the output of the contrasts and save it into a tibble which makes it easier to use in inline code.
...
- `factor_levels` lets you edit the levels of factors you plot, such as renaming or reordering them. You add each factor into a list and you can check the documentation for other options. 
...
One handy feature about this function is it uses <pkg>ggplot2</pkg> in the background, so you can add layers to the initial function like other data visualisation you created. 
...
### Activity 8 - Reporting the results of your factorial ANOVA {#factorial-a8}
